          ...
          <div ref={cardRef}>
            <div className="glass-card p-4 sm:p-6">
              ...
              <div className="space-y-3 sm:space-y-4 mb-4 sm:mb-6">
                {nodes.map((node, i) => (
                  <div key={node.label} className="flex items-center gap-3 sm:gap-4">
                    <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-lg sm:rounded-xl bg-[rgba(79,109,255,0.15)] border border-[rgba(79,109,255,0.3)] flex items-center justify-center">
                      <node.icon className="w-4 h-4 sm:w-5 sm:h-5 text-[#4F6DFF]" />
                    </div>
                    <div className="flex-1">
                      <span className="text-xs sm:text-sm font-medium text-[#F4F6FF]">
                        {node.label}
                      </span>
                      <div className="flex items-center gap-1 sm:gap-2 mt-0.5 sm:mt-1">
                        <span className="w-1.5 h-1.5 rounded-full bg-green-400" />
                        <span className="text-[10px] sm:text-xs text-[#A7B1D8]">Active</span>
                      </div>
                    </div>
                    {i < nodes.length - 1 && (
              ...
              <div className="pt-4 sm:pt-6 border-t border-[rgba(167,177,216,0.1)]">
                <p className="text-[10px] sm:text-xs text-[#A7B1D8] mb-2 sm:mb-3 uppercase tracking-[0.14em]">
                  Available Actions
                </p>
                <div className="flex flex-wrap gap-2">
                  {actions.map(action => (
                    <button
                      key={action.label}
                      className="flex items-center gap-1.5 sm:gap-2 px-2 sm:px-3 py-1.5 sm:py-2 bg-[rgba(79,109,255,0.1)] hover:bg-[rgba(79,109,255,0.2)] border border-[rgba(79,109,255,0.2)] rounded-lg transition-colors"
                    >
                      <action.icon className="w-3 h-3 sm:w-4 sm:h-4 text-[#4F6DFF]" />
                      <span className="text-xs sm:text-sm text-[#F4F6FF]">{action.label}</span>
                    </button>
                  ))}
                </div>
              </div>
